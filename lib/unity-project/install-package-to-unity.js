...
const installPackageToUnity = async (pkgName, opts) => {
  ...
  const pkgSrcRoot = projPath(pkgEntry.package_root)
  const pkgTgtRoot = projPath(pkgEntry.unity_install.path)

  await copySrc2Unity(projPath(pkgEntry.unity_install.src), projPath(pkgEntry.unity_install.path))



  const readmeFiles = (await fs.readdir(pkgSrcRoot)).reduce((acc, cur) => {
    if(cur.match(/^readme/i) !== null) {
      acc.push(cur)
    }
    return acc
  }, [])
  ...
  for(let i = 0; i < readmeFiles.length; i++) {
    let readMeSrc = path.join(pkgSrcRoot, readmeFiles[i])
    let readMeTgt = path.join(pkgTgtRoot, readmeFiles[i])

    if((await fs.lstat(readMeSrc)).isDirectory()) {
      await copySrc2Unity(readMeSrc, readMeTgt)
    }
    else {
