...
const installPackageToUnity = async (pkgName, opts) => {
  ...
  await fs.ensureDirAsync(pkgEntry.unity_install.path)
  await fs.copyAsync(pkgEntry.unity_install.src, pkgEntry.unity_install.path)

  if(await fs.existsAsync(pkgEntry.package_unity_samples)) {
      await fs.ensureDirAsync(pkgEntry.unity_install_samples_path)
      await fs.copyAsync(pkgEntry.package_unity_samples, pkgEntry.unity_install_samples_path)
  }
