...
const installPackageToUnity = async (pkgName, opts) => {
  ...
  await copySrc2Unity(pkgEntry.unity_install.src, pkgEntry.unity_install.path)
  ...
  const readMeSrc = path.join(pkgEntry.package_root, readMeName)

  if(await fs.exists(readMeSrc)) {
    await fs.copy(readMeSrc, path.join(pkgEntry.unity_install.path, readMeName))

    const readMeFilesName = 'readmefiles'
    const readMeFilesSrc = path.join(pkgEntry.package_root, readMeFilesName)
    if(await fs.exists(readMeFilesSrc) && (await fs.lstat(readMeFilesSrc)).isDirectory()) {
      await copySrc2Unity(
        readMeFilesSrc,
        path.join(pkgEntry.unity_install.path, readMeFilesName)
      )
    }
  }

  if(await fs.exists(pkgEntry.package_unity_samples)) {
      await copySrc2Unity(pkgEntry.package_unity_samples,
         pkgEntry.unity_install_samples_path
      )
