...
const copyFromUnity2Pkg = async (pkgName, opts) => {
    ...
    if(!(unitySrc && pkgTgtSrc)) {
        pkgEntry = pkgEntry || await ensureUnpmPackage(pkgName, {...opts})
    ...
    if(unitySamples && !pkgTgtSamples) {
      ...
      pkgTgtSamples = path.join(pkgEntry.clone.path, 'Samples')
    }
    ...
    const copy = async () => {
        ...
        await fs.copy(unitySrc, pkgTgtSrc, {
        ...
        await mergeMetaFiles({
          path_source: unitySrc,
          path_target: pkgTgtSrc
        })

        if(unitySamples && pkgTgtSamples) {
          await fs.copy(unitySamples, pkgTgtSamples, {
              filter: opts.filter ? opts.filter : (src, dst) => {
                  return String(src).match(/^[^.]+$|\.(?!(meta)$)([^.]+$)/)
              },
              overwrite: opts.no_clobber ? false : true
          ...
          await mergeMetaFiles({
            path_source: unitySamples,
            path_target: pkgTgtSamples
          })
        }
