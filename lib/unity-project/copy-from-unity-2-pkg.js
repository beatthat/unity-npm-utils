...
const copyFromUnity2Pkg = async (pkgName, opts) => {
    ...
    const copy = async () => {
        ...
        await copySrcAndMergeMeta(unitySrc, pkgTgtSrc, overwrite, (src, dst) => {
          return filterMeta(src, dst) && String(src).match(readmeRegEx) === null
        })

        if(opts.package_root) {
          const rootFiles = await fs.readdir(opts.package_root)

          for(let i = 0; i < rootFiles.length; i++) {
            if(rootFiles[i].match(readmeRegEx) === null) {
              continue
            }
            if(rootFiles[i].match(nonMetaFileRegEx) === null) {
              continue
            }

            const readmeSrc = path.join(unitySrc, rootFiles[i])
            const readmeTgt = path.join(opts.package_root, rootFiles[i])
...
const copySrcAndMergeMeta = async (src, tgt, overwrite, filter) =>
{
