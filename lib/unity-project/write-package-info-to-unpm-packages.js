...
const writePackageInfoToUnpmPackages = async (pkgName, opts) => {
  ...
  const unpmPackagesExists = await fs.existsAsync(unpmPackagesPath)
   ...
   * would be possible to guess repository info etc based on some basic config in unpm-packages (or unpm-packages if we break that out?)
   ...
   *
{
  ...
  */
  ...
  await fs.writeFileAsync(unpmPackagesPath, JSON.stringify(unpmPackages, null, 2))
