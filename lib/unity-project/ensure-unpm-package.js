...
const ensureUnpmPackage = async (pkgName, opts) => {
    ...
    if(!unpmPkgs.packages[pkgName].unity_install) {
      if(opts.disable_ensure_installed_to_unity) {
        const writeResult = await writePackageInfoToUnpmPackages(pkgName, opts)
        unpmPkgs = writeResult.unpmPackages || await readUnpmPackages(projRoot)
      }
      else {
        if (opts.verbose) {
        ...
        unpmPkgs = installResult.unpmPackages || await readUnpmPackages(projRoot)
      }
    }
    ...
    return (unpmPkgs && unpmPkgs.packages)? unpmPkgs.packages[pkgName] : null
}

module.exports = ensureUnpmPackage
