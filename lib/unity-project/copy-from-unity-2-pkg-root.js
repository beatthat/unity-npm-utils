 ...
 */
const copyFromUnity2PkgRoot = async (unityProjRoot, pkgRoot, opts) => {
    ...
    if(opts.verbose) {
        console.log(`copyFromUnity2PkgRoot unityProjRoot=${unityProjRoot} pkgRoot=${pkgRoot}`)
    }

    var info = await _infoForPkg(pkgRoot, opts)

    return await copyFromUnity2Pkg(unityProjRoot, (info.package)? info.package.name: path.basename(pkgRoot), opts)
}
