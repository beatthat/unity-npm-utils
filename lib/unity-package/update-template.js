...
const updateTemplate = async (pkgRoot, opts) => {
    ...
    await transformPackage({
        ...
        transform: (pkgTemplate, cb) => {
            ...
            pkgAfter.keywords = [ ...pkgBefore.keywords, ...pkgAfter.keywords ]
            pkgAfter.keywords.sort()
            pkgAfter.keywords = pkgAfter.keywords.filter((i, pos) => pos == 0 || i !== pkgAfter.keywords[pos - 1])
