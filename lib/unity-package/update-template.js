...
const updateTemplate = async (pkgRoot, opts) => {
    ...
    if(opts.verbose) {
        console.log(`updateTempate transform op at ${pkgRoot}
            before.scripts=${JSON.stringify(pkgBefore)}
            template.scripts=${JSON.stringify(pkgTemplate)}
            after.scripts=${JSON.stringify(pkgAfter)}`)
    }
    ...
    const srcDirExists = await fs.existsAsync(srcPath)
    ...
    await fs.copyAsync(tmpInstallPath, pkgRoot, (src, dest) => {
        const destName = path.basename(dest)
        if(destName === 'README.md' || destName === 'LICENSE' && fs.existsSync(dest)) {
            return false
        }
        return true
    })
