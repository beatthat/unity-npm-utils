...
const updateTemplate = (pkgRoot, options, callback) => {
    ...
    const promise = new Promise((resolve, reject) => {
        var tmpInstallPath = null;

        _installTemplateToTmp()
        .then(p => {
            tmpInstallPath = p;
            return readPackage(pkgRoot);
        })
        .then(pkgBefore => transformPackage({
        ...
        .then(p => setPackageName(tmpInstallPath)) // call setPackageName to make sure ./test/package.json dependency on main module is set correctly
        .then(pkgNameSet => addSrcFiles(tmpInstallPath,
            [{name: 'INSTALL.txt', content: 'Placeholder file copies to unity-project'}]
        ))
        .then(addedPlaceholderSrc => fs.copyAsync(tmpInstallPath, pkgRoot))
