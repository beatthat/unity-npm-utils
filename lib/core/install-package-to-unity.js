...
const installPackageToUnity = async (pkgName, opts) => {
  ...
  await writePackageInfoToUnpmLocal(pkgName, {
    ...opts,
    project_root: projRoot
  })

  dirExists = await fs.ensureDirAsync(info.unity_install_path)
  await fs.copyAsync(info.package_unity_src, info.unity_install_path)
