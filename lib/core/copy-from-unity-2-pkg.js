 ...
 */
const copyFromUnity2Pkg = async (pkgName, opts, callback) => {

    opts = opts || {};

    opts.project_root = opts.project_root || appRoot

    const pkgEntry = await ensureUnpmLocalPackage(pkgName, opts)

    if (opts.verbose) {
        console.log(`copyFromUnity2Pkg(${pkgName}) read unpm-local package entry:\n ${JSON.stringify(pkgEntry, null, 2)}`)
    }

//TODO: need all the unity-install-path stuff from _infoForPkg to move to unpm-local.json

    const unitySrc = pkgEntry.unity_install.path;
    const pkgTgtSrc = opts.package_target_path || path.join(pkgEntry.clone.path, 'src');
