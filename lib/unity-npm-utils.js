...
const installPlugin = function(moduleRoot, pkgScope, srcPkg) {
    ...
    // expecting the module root will be as (unity-project)/node_modules/$thisModule...
 ...
 */
const copyFromUnity2Pkg = (unitySrcRoot, pkgName, options, callback) => {
...
const copyUnity2LinkedClone = (unityRoot, pkgName, options, callback) => {
    ...
    const promise = new Promise((resolve, reject) => {
        ...
        if (options.verbose) {
            console.log(`read pkg with name ${pkg.name}`);
        }
...
//
// /**
//  * Sync the (local/npm pack'ed) path for a 'test' module
//  * within your Unity npm package.
//  *
//  * A typical Unity npm package will have a 'test' module under its root,
//  * where the idea is that the user installs the test module,
//  * edits the package source in Unity
//  * and then syncs changes back to the main module source.
//  *
//  * In order to avoid having to publish changes
//  * to make them available to this test module,
//  * the test module usually references a local tar of the module
//  * create via <code>npm pack</code>.
//  *
//  * That npm-pack'ed tgz file has the current module version
//  * as part of its filename, and no
//  *
//  *
//  *
//  * Once this connection is established, changes made to the (module's) source
...
//  *      Defaults to 2 directories above moduleRoot param
//  */
// const syncLocalModuleVersion = (modRoot, options, callback) => {
//     if (typeof(options) === 'function') {
//         callback = options;
//         options = {};
//     }
