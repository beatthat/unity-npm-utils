...
const copyFromUnity2Pkg = (unitySrcRoot, pkgName, options, callback) => {
    ...
    const promise = new Promise((resolve, reject) => {
        ...
        _infoForPkg(modRoot, options, (infoErr, info) => {
            ...
            const pkgTgtSrt = info.package_unity_src;
            ...
            const copy = () => {
                ...
                if (options.verbose) {
                    console.log(`will copy from ${unitySrc} to ${pkgTgtSrt} with options ${JSON.stringify(options)}`)
                }

                ncp(unitySrc, pkgTgtSrt, ncpOpts, (cpErr) => {
                    if (cpErr) {
                        console.error(cpErr);
            ...
            if (options.overwrite) {
                ...
                const pkgTgtDel = info.package_del_tgt = path.join(pkgTgtSrt, '*');
