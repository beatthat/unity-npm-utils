...
/**
 * Install an unity package from node_modules/$package_name
 ...
 *
 * @param {string} options.no_clobber - passed to ncp as 'clobber'
 *      By default, sets clobber to TRUE, so use no_clobber: true
 *      to disable clobbering.
 */
const syncPlugin2Src = function(moduleRoot, options) {
    options = options? options: {};

    const srcPkg = options.src_path ? options.src_path : "src";
    const pkgScope = options.package_scope ? options.package_scope : process.env.npm_package_scope;
    const pkgName = options.package_name ? options.package_name : process.env.npm_package_name;

    const tgtSrc = path.join(moduleRoot, srcPkg);

    // expecting the module root will be as $unityProject/node_modules/$thisModule...
    const unitySrc = path.join(moduleRoot, '..', '..', 'Assets', 'Plugins', pkgScope, pkgName);
    ...
    ncp(unitySrc, tgtSrc, ncpOpts, function(err) {
        if (err) {
            console.error(err);
            process.exit(1);
        }
