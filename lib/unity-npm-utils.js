...
const _infoForPkg = (modRoot, options, callback) => {
    ...
    const promise = new Promise((resolve, reject) => {
        ...
        info.package = require(pkgPath);

        if (!(info.package && info.package.name)) {
            return reject("Not a valid package.json at " + pkgPath);
        }

        const unityInstallPath = [modRoot, '..', '..', 'Assets'];
        ...
        info.package_unity_src_rel = (info.package.config && info.package.config.src_path) ?
            info.package.config.src_path : "src";

        info.package_unity_src = path.join(modRoot, info.package_tgt_src_rel, info.package.name);
    ...
    const promise = new Promise((resolve, reject) => {
        ...
        _infoForPkg(modRoot, options, (infoErr, info) => {
            ...
            mkdirp(info.unity_install_path, (mkdirErr) => {
                ...
                // Copy files
                ncp(info.package_unity_src, info.unity_install_path, function(err) {
                    if (cpErr) {
                        console.error(cpErr);
                        return reject(cpErr);
                    }
