 ...
 */
const syncPlugin2Src = function(unityRoot, tgtPkgRoot, options) {
    options = options ? options : {};

    // expecting the module root will be as $unityProject/node_modules/$thisModule...

    const unitySrc = path.join(unitySrcRoot, '..', '..', 'Assets', 'Plugins',
        options.package_scope ? options.package_scope : process.env.npm_package_scope,
        options.package_name ? options.package_name : process.env.npm_package_name);

    const pkgTgtSrc = path.join(pkgTgtRoot, options.src_path ? options.src_path : "src");
    ...
    ncp(unitySrc, pkgTgtSrc, ncpOpts, function(err) {
        if (err) {
            console.error(err);
            process.exit(1);
        }
