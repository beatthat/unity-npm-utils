...
describe("unityProject.findUnityPathForPackage", () => {
    ...
    it("- can guess unity path when a package's scope is defined in unpm-package.json", async function() {
      ...
      const unityPath = await unpm.unityProject.findUnityPathForPackage(testPkgName, {
        project_root: testProjPath,
        verbose: VERBOSE
      })

      expect(unityPath).to.equal(
          path.join(testProjPath, 'Assets', 'Plugins', 'packages', 'beatthat', testPkgName)
      )
    })
